...
parser.add_argument("--name",type=str,default="try",
...
parser.add_argument("--patience",type=int,default=-1,
    help="patience of early stopping. Use -1 to desactivate early stopping.",)
parser.add_argument("--batch_size",type=int,default=256,
    choices=range(1, 1025),metavar="[1-1024]",help="Batch size.",)
parser.add_argument("--epochs",type=int,default=50,
    choices=range(1, 1001),metavar="[1-1000]",help="Maximum number of epochs.",)
parser.add_argument("--num_workers",type=int,default=1,
    choices=range(1, 65),metavar="[1-64]",help="Number of workers.",)
parser.add_argument("--seed", type=int, default=42, help="Random seed.")

parser.add_argument("--lr", type=float, default="0.01", help="Initial learning rate.")
parser.add_argument("--lr_factor", type=float, default="0.1", help="Factor of Scheduler.")
parser.add_argument("--momentum", default=0, type=float, help="Momentum(usuallly 0.9).")
parser.add_argument("--weight_decay", default=0, type=float, help="Weight decay(usually 0.01 of base_lr).")
parser.add_argument("--nesterov", help="Enable Nesterov momentum.", action="store_true")
parser.add_argument("--path_classes",default=os.path.join("models", "classes.json"),type=str,
...
def main():
    ...
    if not args.name:
        ...
        args.name = f"{timestamp}_{args.model}"
    # Create the folder to store the results of all the experiments
    args.experiment_path = os.path.join("experiments", args.name)
    ...
    # Add random cropping to the list of transformations (fill in with padding)
    if args.random_crop:
        train_transforms.insert(0, transforms.RandomCrop(28, padding=4))

    # Add random erasing to the list of transformations (cover with blank)
    ...
    if args.rgb:
        ...
        train_transforms.append(convert_to_RGB)
        test_transforms.append(convert_to_RGB)
    ...
    train_set_length = int(
        len(train_dataset) * (100 - args.percent) / 100)
    ...
    num_trainable_parameters = utils.count_parameters(model, only_trainable_parameters=True)
    ...
    # scheduler = ReduceLROnPlateau(optimizer,factor=args.lr_factor,patience=50)

    # Print information from the parameters and model before training
    print(f"Loaded {args.model} with {num_trainable_parameters} trainable parameters (GPU: {use_cuda}).")
    print(args)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Early stopping
            ...
            print(f"No improvement for the last {epochs_without_improvement} epochs, \
                stopping the training (best accuracy: {100 * best_accuracy:05.2f}).")
            break
    ...
    print(f"Best {args.model} model has an accuracy of {100 * test_accuracy_top1:05.2f} on the Fashion MNIST test set.")
...
def train(
    ...
    for i, data in enumerate(progress_bar):
        ...
        progress_bar.set_description(f"Epoch {epoch + 1}/{args.epochs} Train")
